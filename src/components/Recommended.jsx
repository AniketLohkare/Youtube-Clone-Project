...
const Recommended = ({ videoData }) => {
  ...
  const fetchRecommendedVideos = async () => {
    ...
    const fetchVideosUsingCategoryIdUrl = `https://youtube.googleapis.com/youtube/v3/videos?part=snippet%2CcontentDetails%2Cstatistics&chart=mostPopular&maxResults=40&regionCode=US&videoCategoryId=${categoryId}&key=${API_KEY}`
  ...
  return (
    <div id='recommended-videos' className='flex basis-3/10 flex-col gap-0.5'>
      {recommendedVideos.map((videoData) => {
        return (
          <Link
            key={videoData.id}
            to={`/video/${videoData.id}`}
            className='flex gap-3 p-1.5 hover:bg-gray-200'
          >
            <img
